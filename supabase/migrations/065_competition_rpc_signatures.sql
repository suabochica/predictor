-- 065_competition_rpc_signatures.sql
-- Phase 2, part 3: the three RPCs whose signature HAS to change, because
-- nothing in their existing arguments identifies a competition.
--
--   run_auto_bids(p_round)              -> (p_round, p_competition_id)
--   auto_complete_squads()              -> (p_competition_id)
--   set_country_eliminated(text, bool)  -> (text, bool, p_competition_id)
--
-- Each old overload is DROPped explicitly. Leaving it in place would make
-- PostgREST fail with "could not choose the best candidate function" on every
-- call, and would leave a live, unscoped copy of exactly the writes this
-- migration exists to contain.
--
-- Why these three and not "p_competition_id everywhere for symmetry": a
-- client-supplied competition id is an authorization input that then has to be
-- validated. Every other RPC derives its competition from a player/team/matchday
-- id the caller was already authorized for, which is strictly safer. These three
-- have nothing to derive from — they scan `teams`, `players` and `auction_state`
-- wholesale — and all three are admin-only, which bounds the exposure.
--
-- What each would have done unscoped once UCL exists:
--   run_auto_bids        — read the wrong auction_state row, then place proxy
--                          bids for every auto-bid team in BOTH competitions.
--   auto_complete_squads — fill World Cup squads from the World Cup player pool
--                          while the admin was finishing the UCL auction.
--   set_country_eliminated — flip is_eliminated on every player sharing a
--                          country_code across competitions (club codes and
--                          country codes share the column).

-- ── run_auto_bids ─────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS run_auto_bids(integer);
...
BEGIN
  ...
  FROM auction_state
  WHERE competition_id = p_competition_id                        -- [COMPETITION]
  ORDER BY id LIMIT 1;
  ...
  FOR r_team IN
    ...
    WHERE t.auto_bid_enabled = true
      AND t.competition_id = p_competition_id                    -- [COMPETITION]
  LOOP
    ...
    -- [COMPETITION] round numbers repeat across competitions.
    ...
    SELECT EXISTS (
      ...
      WHERE ab.user_id = r_team.user_id AND ab.round_number = p_round
        AND ab.competition_id = p_competition_id                 -- [COMPETITION]
        AND p.position = 'GK'
    ) INTO v_has_gk;
    ...
    FOR r_target IN
      ...
      WHERE pt.user_id = r_team.user_id
        AND pt.competition_id = p_competition_id                 -- [COMPETITION]
...
-- ── auto_complete_squads ──────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS auto_complete_squads();

CREATE OR REPLACE FUNCTION auto_complete_squads(p_competition_id integer)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT max_squad_size INTO v_max_squad FROM competitions WHERE id = p_competition_id;
  IF v_max_squad IS NULL THEN
    RAISE EXCEPTION 'Unknown competition %.', p_competition_id;
  END IF;
  ...
  FOR r_team IN
    ...
    FROM teams t
    WHERE t.competition_id = p_competition_id                    -- [COMPETITION]
      AND (SELECT COUNT(*) FROM team_players WHERE team_id = t.id) < v_max_squad
  LOOP
    SELECT COUNT(*) INTO v_owned FROM team_players WHERE team_id = r_team.team_id;
    v_needed := v_max_squad - v_owned;
    ...
    IF NOT v_has_gk AND v_needed > 0 THEN
      ...
      FROM players p
      WHERE p.position = 'GK'
        AND p.competition_id = p_competition_id                  -- [COMPETITION]
      ...
      IF v_gk_id IS NOT NULL THEN
        ...
        VALUES (r_team.team_id, v_gk_id, v_gk_price, p_competition_id)
    ...
    FOR r_player IN
      ...
      FROM players p
      WHERE p.competition_id = p_competition_id                  -- [COMPETITION]
        AND NOT EXISTS (SELECT 1 FROM team_players WHERE player_id = p.id)
      ORDER BY random()
    LOOP
      ...
      INSERT INTO team_players (team_id, player_id, acquisition_price, competition_id)
      VALUES (r_team.team_id, r_player.id, r_player.current_price, p_competition_id)
      ON CONFLICT (team_id, player_id) DO NOTHING;
...
GRANT EXECUTE ON FUNCTION auto_complete_squads(integer) TO authenticated;


-- ── set_country_eliminated ────────────────────────────────────────────────────
-- players.country_code holds a FIFA country code today and will hold a club code
-- for the UCL. Codes can collide across competitions, so the UPDATE must be
-- scoped even though the argument looks like it identifies one entity.
DROP FUNCTION IF EXISTS set_country_eliminated(text, boolean);

CREATE OR REPLACE FUNCTION set_country_eliminated(
  p_country_code   text,
...
BEGIN
  ...
  UPDATE players
  SET is_eliminated = p_eliminated
  WHERE country_code = p_country_code
    AND competition_id = p_competition_id;                       -- [COMPETITION]
END $$;

GRANT EXECUTE ON FUNCTION set_country_eliminated(text, boolean, integer) TO authenticated;
