-- 047_accumulate_group_transfers.sql
-- Replace execute_transfer so group-stage transfers pool cumulatively:
-- max = TRANSFER_CAP_GROUP × (# of group matchdays with id ≤ active), counted across all of them.
-- Knockout keeps per-MD cap (TRANSFER_CAP_KO). Everything else in the function is unchanged.
